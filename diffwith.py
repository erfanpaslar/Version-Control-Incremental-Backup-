## I downloaded this code and try to convert it to c but i couldn't.
## so i wrote other parts of project :) 
        ...
        for k in range(-d, d + 1, 2):
           ...
           # ///////////////////////////////////////////////////////////
            ...
            if 1 <= y <= b_max and go_down:
                history.append(Insert(b_lines[y-1]))
            elif 1 <= x <= a_max:
                history.append(Remove(a_lines[x-1]))
            ...
            while x < a_max and y < b_max and a_lines[x] == b_lines[y]:
                x += 1
                y += 1
                history.append(Keep(a_lines[x-1]))
            if x >= a_max and y >= b_max:
                # If we're here, then we've traversed through the bottom-left corner,
                # and are done.
                return history
            else:
                frontier[k] = Frontier(x, history)###########this is where i think id should add

    assert False, 'Could not find edit script'

def main(a_file, b_file, logFile):
    # try:
...
a_file, b_file, logFile = input().split("?")

print(a_file)
print(b_file)
print(logFile)
main(a_file, b_file, logFile)
